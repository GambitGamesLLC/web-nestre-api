/**
 * index.js
 * @file Frontend script for the create-team-code example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to create a new team code for an organization via the Admin App API.
...
function RunButtonClicked() {
    ...
    RunTest(organizationId, teamCode, authToken);
}
 ...
 * @param {import('../../../src/admin-app/admin-app-types.js').TeamCode} teamCode 
 * @param {string} authToken 
 */
async function RunTest(organizationId, teamCode, authToken) {
    ...
    try {
        Log(`Creating team code for organization: ${organizationId}...`);
        
        const response = await NestreApiManager.GetInstance().adminAppApi.CreateTeamCodeForOrganization(organizationId, teamCode);
