/**
 * index.js
 * @file Frontend script for the get-member-referral-code-stats example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch referral code statistics for an organization's members via the Admin App API.
...
function RunButtonClicked() {
    ...
    RunTest(organizationId, fromDate, toDate, aggregateCodes, authToken);
}
...
async function RunTest(organizationId, fromDate, toDate, aggregateCodes, authToken) {
    ...
    try {
        Log(`Fetching member referral code stats for organization ${organizationId}...`);
        
        const response = await NestreApiManager.GetInstance().adminAppApi.GetMemberReferralCodeStats(organizationId, fromDate, toDate, aggregateCodes);
