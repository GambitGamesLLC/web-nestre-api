/**
 * index.js
 * @file Frontend script for the delete-organization example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to delete an organization via the Admin App API.
...
async function RunTest(organizationId, authToken) {
    ...
    try {
        Log(`Deleting organization: ${organizationId}...`);
        await NestreApiManager.GetInstance().adminAppApi.DeleteOrganization(organizationId);
        Log('✅ Test successful! Organization deleted.');
    } catch (error) {
