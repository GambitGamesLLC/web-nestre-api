/**
 * index.js
 * @file Frontend script for the get-daily-workout-recommendation example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch a daily workout recommendation. 
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        Log(`Fetching daily workout recommendation for user: ${userId}...`);
        
        // Because of JSDoc, you get autocompletion here in VS Code!
        const recommendation = await NestreApiManager.GetInstance().dailyWorkoutApi.GetDailyWorkoutRecommendation(userId);

        Log('✅ Test successful!');
        Log('Daily Workout Recommendation Loaded:');
