/**
 * index.js
 * @file Frontend script for the create-music-content-interaction example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to record a user's interaction with music content.
...
/**
 * Textarea for the music interaction data
 * @type{HTMLElement | null}
 */
...
//------------------------------------------------------------//
{
    ...
    try {
        ...
        Log(`❌ Error: Invalid JSON in Music Interaction data. ${e.message}`);
        return;
    }
    ...
    try {
        Log(`Recording music interaction for user: ${userId}...`);
        
        const confirmation = await NestreApiManager.GetInstance().contentInteractionApi.CreateMusicContentInteraction(userId, interactionObject);
