/**
 * index.js
 * @file Frontend script for the create-mindset-minute-content-interaction example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to record a user's interaction with mindset minute content.
...
/**
 * Textarea for the mindset minute interaction data
 * @type{HTMLElement | null}
 */
...
//------------------------------------------------------------//
{
    ...
    try {
        ...
        Log(`❌ Error: Invalid JSON in Mindset Minute Interaction data. ${e.message}`);
        return;
    }
    ...
    try {
        Log(`Recording mindset minute interaction for user: ${userId}...`);
        
        const confirmation = await NestreApiManager.GetInstance().contentInteractionApi.CreateMindsetMinuteContentInteraction(userId, interactionObject);
