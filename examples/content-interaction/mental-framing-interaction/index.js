/**
 * index.js
 * @file Frontend script for the create-mental-frame-content-interaction example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to record a user's interaction with mental frame content.
...
/**
 * Textarea for the mental frame interaction data
 * @type{HTMLElement | null}
 */
...
//------------------------------------------------------------//
{
    ...
    try {
        ...
        Log(`❌ Error: Invalid JSON in Mental Frame Interaction data. ${e.message}`);
        return;
    }
    ...
    try {
        Log(`Recording mental frame interaction for user: ${userId}...`);
        
        const confirmation = await NestreApiManager.GetInstance().contentInteractionApi.CreateMentalFrameContentInteraction(userId, interactionObject);
