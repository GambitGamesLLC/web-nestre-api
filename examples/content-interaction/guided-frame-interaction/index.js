/**
 * index.js
 * @file Frontend script for the create-guided-frame-content-interaction example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to record a user's interaction with guided frame content.
...
/**
 * Textarea for the guided frame interaction data
 * @type{HTMLElement | null}
 */
...
//------------------------------------------------------------//
{
    ...
    try {
        ...
        Log(`❌ Error: Invalid JSON in Guided Frame Interaction data. ${e.message}`);
        return;
    }
    ...
    try {
        Log(`Recording guided frame interaction for user: ${userId}...`);
        
        const confirmation = await NestreApiManager.GetInstance().contentInteractionApi.CreateGuidedFrameContentInteraction(userId, interactionObject);
