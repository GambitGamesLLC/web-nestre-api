/**
 * index.js
 * @file Frontend script for the get-members example web page.
...
//------------------------------------------------------------//
{
    ...
    try {
        Log(`Fetching members for organization: ${organizationId}...`);
        
        const members = await NestreApiManager.GetInstance().organizationApi.GetMembers(organizationId, tag, emailSearch);

        Log('✅ Test successful!');
        Log('Organization Members Loaded:');
        
        const membersString = JSON.stringify(members, null, 2);
        outputDiv.innerHTML += `<pre>${membersString}</pre>`;
