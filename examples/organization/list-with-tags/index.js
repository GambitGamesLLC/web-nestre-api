/**
 * index.js
 * @file Frontend script for the list-with-tags example web page.
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        ...
        const organizations = await NestreApiManager.GetInstance().organizationApi.ListOrganizations();

        Log('✅ Test successful!');
        Log('Organizations Loaded:');
        
        // Display result in a readable format
        const orgsString = JSON.stringify(organizations, null, 2);
        outputDiv.innerHTML += `<pre>${orgsString}</pre>`;
