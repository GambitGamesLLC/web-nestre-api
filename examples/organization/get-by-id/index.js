/**
 * index.js
 * @file Frontend script for the get-by-id example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch detailed information for a single organization by its ID.
...
//------------------------------------------------------------//
{
    ...
    try {
        Log(`Fetching organization data for: ${organizationId}...`);
        
        const organizationData = await NestreApiManager.GetInstance().organizationApi.GetOrganizationById(organizationId);

        Log('✅ Test successful!');
        Log('Organization Data Loaded:');
        
        const orgDataString = JSON.stringify(organizationData, null, 2);
