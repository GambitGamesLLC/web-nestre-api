/**
 * index.js
 * @file Frontend script for the get-randomized-questions example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch randomized assessment questions. 
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        Log(`Fetching randomized assessment questions for user: ${userId}...`);
        
        const randomizedQuestions = await NestreApiManager.GetInstance().assessmentApi.GetRandomizedAssessmentQuestions(userId);

        Log('✅ Test successful!');
        Log('Randomized Assessment Questions Loaded:');
        
        // Display result in a readable format
        const questionsString = JSON.stringify(randomizedQuestions, null, 2);
        outputDiv.innerHTML += `<pre>${questionsString}</pre>`;
