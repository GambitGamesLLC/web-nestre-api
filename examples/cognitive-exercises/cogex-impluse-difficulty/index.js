/**
 * index.js
 * @file Frontend script for the cogex-impulse-difficulty example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch Impulse difficulty settings. 
...
async function RunTest(userId, level, authToken) {
    ...
    try {
        Log(`Fetching Impulse difficulty for user: ${userId} at level: ${level}...`);
        
        const impulseDifficulty = await NestreApiManager.GetInstance().cognitiveExercisesApi.GetImpulseDifficulty(userId, level);

        Log('✅ Test successful!');
        Log('Impulse Difficulty Loaded:');
        
        const resultString = JSON.stringify(impulseDifficulty, null, 2);
