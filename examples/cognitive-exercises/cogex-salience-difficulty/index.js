/**
 * index.js
 * @file Frontend script for the cogex-salience-difficulty example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch Salience difficulty settings. 
...
async function RunTest(userId, level, authToken) {
    ...
    try {
        Log(`Fetching Salience difficulty for user: ${userId} at level: ${level}...`);
        
        const salienceDifficulty = await NestreApiManager.GetInstance().cognitiveExercisesApi.GetSalienceDifficulty(userId, level);

        Log('✅ Test successful!');
        Log('Salience Difficulty Loaded:');
        
        const resultString = JSON.stringify(salienceDifficulty, null, 2);
