/**
 * index.js
 * @file Frontend script for the get-current-session-interactions-for-cognitive-exercise example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch the current session interactions for a specific cognitive exercise. 
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        Log(`Fetching current session interactions for exercise: ${cogexId} for user: ${userId}...`);
        
        // Because of JSDoc, you get autocompletion here in VS Code!
        const sessionInteractions = await NestreApiManager.GetInstance().cognitiveExercisesApi.GetCurrentSessionInteractions(userId, cogexId);

        Log('✅ Test successful!');
        Log('Session Interactions Loaded:');
        
        // Display result in a readable format
        const sessionInteractionsString = JSON.stringify(sessionInteractions, null, 2);
        outputDiv.innerHTML += `<pre>${sessionInteractionsString}</pre>`;
