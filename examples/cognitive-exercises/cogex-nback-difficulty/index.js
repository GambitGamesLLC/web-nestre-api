/**
 * index.js
 * @file Frontend script for the cogex-nback-difficulty example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch N-Back difficulty settings. 
...
async function RunTest(userId, level, authToken) {
    ...
    try {
        Log(`Fetching N-Back difficulty for user: ${userId} at level: ${level}...`);
        
        const nBackDifficulty = await NestreApiManager.GetInstance().cognitiveExercisesApi.GetNBackDifficulty(userId, level);

        Log('✅ Test successful!');
        Log('N-Back Difficulty Loaded:');
        
        const resultString = JSON.stringify(nBackDifficulty, null, 2);
