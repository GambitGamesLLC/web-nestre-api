/**
 * index.js
 * @file Frontend script for the cogex-nback-sequence example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch an N-Back sequence. 
...
async function RunTest(userId, level, version, authToken) {
    ...
    try {
        Log(`Fetching N-Back sequence for user: ${userId} at level: ${level} with version: ${version}...`);
        
        const nBackSequence = await NestreApiManager.GetInstance().cognitiveExercisesApi.GetNBackSequence(userId, level, version);

        Log('✅ Test successful!');
        Log('N-Back Sequence Loaded:');
        
        const resultString = JSON.stringify(nBackSequence, null, 2);
