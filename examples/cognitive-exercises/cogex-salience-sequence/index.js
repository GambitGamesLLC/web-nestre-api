/**
 * index.js
 * @file Frontend script for the cogex-salience-sequence example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch a Salience sequence. 
...
function RunButtonClicked() {
    ...
    RunTest(userId, level, version, authToken);
}
...
async function RunTest(userId, level, version, authToken) {
    if (!userId) {
    ...
    try {
        Log(`Fetching Salience sequence for user: ${userId} at level: ${level} with version: ${version}...`);
        
        const salienceSequence = await NestreApiManager.GetInstance().cognitiveExercisesApi.GetSalienceSequence(userId, level, version);

        Log('✅ Test successful!');
        Log('Salience Sequence Loaded:');
        
        const resultString = JSON.stringify(salienceSequence, null, 2);
