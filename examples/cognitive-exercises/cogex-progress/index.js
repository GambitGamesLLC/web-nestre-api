/**
 * index.js
 * @file Frontend script for the get-current-level-for-cognitive-exercise example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch the current progress for a specific cognitive exercise. 
...
//#region PRIVATE - LIFECYCYCLE - ADD EVENT LISTENER - DOM CONTENT LOADED
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        Log(`Fetching user progress for exercise: ${cogexId} for user: ${userId}...`);
        
        // Because of JSDoc, you get autocompletion here in VS Code!
        const userProgress = await NestreApiManager.GetInstance().cognitiveExercisesApi.GetCurrentLevelForCognitiveExercise(userId, cogexId);

        Log('✅ Test successful!');
        Log('User Progress Loaded:');
        
        // Display result in a readable format
        const userProgressString = JSON.stringify(userProgress, null, 2);
        outputDiv.innerHTML += `<pre>${userProgressString}</pre>`;
