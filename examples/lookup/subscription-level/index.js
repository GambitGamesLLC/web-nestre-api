/**
 * index.js
 * @file Frontend script for the lookup/subscription-level example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch subscription level options. 
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        Log(`Fetching subscription level options...`);
        
        // Because of JSDoc, you get autocompletion here in VS Code!
        const subscriptionOptions = await NestreApiManager.GetInstance().lookupApi.GetSubscriptionOptions();

        Log('✅ Test successful!');
        Log('Subscription Level Options Loaded:');
        
        // Display result in a readable format
        const optionsString = JSON.stringify(subscriptionOptions, null, 2);
