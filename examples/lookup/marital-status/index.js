/**
 * index.js
 * @file Frontend script for the lookup/marital-status example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch marital status options. 
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        Log(`Fetching marital status options...`);
        
        // Because of JSDoc, you get autocompletion here in VS Code!
        const maritalOptions = await NestreApiManager.GetInstance().lookupApi.GetMaritalOptions();

        Log('✅ Test successful!');
        Log('Marital Status Options Loaded:');
        
        // Display result in a readable format
        const optionsString = JSON.stringify(maritalOptions, null, 2);
