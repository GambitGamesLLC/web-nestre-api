/**
 * index.js
 * @file Frontend script for the get-content example web page.
 * @description This script demonstrates how to use the `web-nestre-api` library 
 * to fetch mental framing content IDs for a user.
...
//------------------------------------------------------------//
{
    ...
    try 
    {
        Log(`Fetching mental framing content IDs for user: ${userId}...`);
        
        // Because of JSDoc, you get autocompletion here in VS Code!
        const contentIds = await NestreApiManager.GetInstance().mentalFramingApi.GetMentalFramingContentIds(userId);

        Log('✅ Test successful!');
        Log('Mental Framing Content IDs Loaded:');
        
        // Display result in a readable format
        const contentIdsString = JSON.stringify(contentIds, null, 2);
        outputDiv.innerHTML += `<pre>${contentIdsString}</pre>`;
