...
handlers.push
(
  http.get(`${API_BASE_URL}/v${API_VERSION}/user/:userId`, ({ params }) => 
  {
    const { userId } = params;

    // For the happy path, we'll return the mock profile
    // if the userId matches our test userId.
    if (userId === USER_ID) {
      return HttpResponse.json(mockBasicUserProfile);
    }

    // If the userId does not match, return a 404 error.
    if (userId !== USER_ID) {
...
handlers.push
(
  http.patch(`${API_BASE_URL}/v${API_VERSION}/user/:userId`, async({ request, params }) => 
  {
    const { userId } = params;
...
handlers.push
(
  http.get(`${API_BASE_URL}/v${API_VERSION}/user/:userId/profile`, async({ params }) => 
  {
    const { userId } = params;
...
handlers.push
(
  http.post(`${API_BASE_URL}/v${API_VERSION}/user/:userId/referral-code`, async({ params }) => 
  {
    const { userId } = params;
